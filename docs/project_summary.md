...
5. **认证架构改进**
   - 将登录认证系统完全重构为使用REST API：
     - 创建了`AuthApiClient`类，使用Dio库处理HTTP请求
     - 使用JSON作为数据格式，更易于调试和维护
     - 提供了完整的错误处理和模拟模式支持
   - 职责分离：
     - 认证/登录使用REST API，安全且标准
     - 实时通信使用Socket.io，在登录成功后才升级到认证状态
     - 清晰区分用户认证和实时通信的责任边界
   - 技术优化：
     - 添加了网络请求日志和详细的错误报告
     - 实现了更精细的状态管理
...
1. **Socket.io连接简化**：
   - 移除了匿名连接和连接升级的概念
   - 统一为单一的connect方法，支持可选的认证参数
   - 登录前使用无认证连接，登录后断开并重新连接（带认证信息）

2. **模拟登录改进**：
   - 重新设计模拟登录流程，使用标准REST API流程
   - 模拟账号使用固定的手机号(13800138000)和验证码(123456)
   - 所有登录（包括模拟登录）都遵循相同的AuthCubit处理流程
   - 确保模拟环境和生产环境的行为一致性

3. **认证流程标准化**：
   - REST API处理身份验证和令牌获取
   - 获取令牌后，Socket.io连接才使用认证信息
   - 清晰分离认证逻辑和通信逻辑
