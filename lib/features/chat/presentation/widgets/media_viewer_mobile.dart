...
class _ImageViewerPageState extends State<ImageViewerPage> {
  ...
  ImageProvider _getImageProvider() {
    try {
      ...
      // 如果是Web平台且没有网络URL，尝试从路径推断
      if (kIsWeb && _effectivePath.startsWith('http')) {
...
class _VideoViewerPageState extends State<VideoViewerPage> {
  ...
  Future<void> _initializePlayer() async {
    ...
    try {
      ...
      } else if (kIsWeb && _effectivePath.startsWith('http')) {
        // Web平台且路径是URL
        _videoPlayerController =
            VideoPlayerController.networkUrl(Uri.parse(_effectivePath));
        _logger.d('Web平台使用URL初始化视频', extra: {'url': _effectivePath});
      } else {
  ...
  static void openMedia(BuildContext context, Message message,
    ...
    if (mediaUrl != null && mediaUrl.isNotEmpty) {
      if (mediaUrl.startsWith('file://')) {
        effectivePath = kIsWeb ? mediaUrl : mediaUrl.substring(7);
      } else {
