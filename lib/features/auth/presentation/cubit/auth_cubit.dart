...
class AuthCubit extends Cubit<AuthState> {
  ...
  void updatePhoneNumber(String phoneNumber) {
    ...
    _logger.info('更新手机号: $phoneNumber');
  ...
  Future<void> sendVerificationCode() async {
    ...
    if (currentState is AuthFormState) {
      if (currentState.phoneNumber?.length != 11) {
        _logger.info('手机号错误: ${currentState.phoneNumber}');
      ...
      try {
        ...
        _logger.info('验证码已发送，倒计时: $countdownDuration');

        _startCountdown();
      } catch (e) {
        _logger.info('发送验证码错误: $e');
  ...
  Future<void> login(bool isQuickLogin) async {
    final currentState = state;
    _logger.info('登录请求 - 快捷登录: $isQuickLogin');
    if (currentState is AuthFormState) {
      try {
        // 验证手机号
        ...
        if (isQuickLogin) {
          ...
          _logger.info('使用验证码登录: ${currentState.verificationCode}');
          // TODO: 实现验证码登录API调用
        } else {
          // 密码登录
          ...
          _logger.info('使用密码登录: ${currentState.password}');
          // TODO: 实现密码登录API调用
        }
  ...
  Future<void> register(String phoneNumber, String password, String verificationCode) async {
    _logger.info('注册请求: $phoneNumber');
    try {
      // 验证手机号
      ...
      if (phoneNumber.length != 11) {
        _logger.info('手机号错误: $phoneNumber');
        throw '请输入正确的手机号码';
      }
